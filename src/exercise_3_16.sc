...
object List {
  ...
  def sum2(ns: List[Int]) =
    foldRigth(ns, 0)((x,y) => x + y)

  def product2(ns: List[Double]) =
    foldRigth(ns, 1.0)(_ * _)

  //
  // can we halt recursion if we see 1.0?
  // not possible, strict evolution done for f's arguments
  def product3_7(ns: List[Double]) =
    foldRigth(ns, 1.0)((x,y) => x * y)

  def length[A](as: List[A]): Int =
    foldRigth(as, 0)((_,acc) => 1 + acc)

  def foldRigth[A,B](as: List[A], z: B)(f: (A,B) => B): B =
    as match {
      case Nil => z
      case Cons(x, xs) => f(x, foldRigth(xs, z)(f))
    }
  ...
  def append2[A](a1: List[A], a2: List[A]): List[A] = {
    foldRigth(a1, a2)(Cons(_,_))
  }

  def concat[A](l: List[List[A]]): List[A] =
    foldRigth(l, Nil:List[A])(append)

  def addOneToEach(l: List[Int]): List[Int] =
    foldRigth(l, Nil:List[Int])((h, t) => Cons(h+1, t))
